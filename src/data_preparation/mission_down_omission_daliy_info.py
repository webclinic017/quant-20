...
'''
任务：检查指定股票在特定时间段内日交易数据是否下载完成，如果没有下载则进行下载
前置条件：t_daily_info_down_log中已经检查各交易日的数据是否下载

步骤:
    1、执行mission_down_day_info.py下载指定时间区间内交易日信息
    2、执行mission_check_daily_info.py核对
'''
...
def mission(start_date, end_date, ts_codes):
    for ts_code in ts_codes:
        logger.info("开始：进行任务，股票：%s，在%s 到 %s期间的日交易数据检测及补充！", ts_code, start_date, end_date)
        process_single_stock(start_date, end_date, ts_code)
        logger.info("结束：结束任务，股票：%s，在%s 到 %s期间的日交易数据检测及补充完毕！", ts_code, start_date, end_date)
        # break
        time.sleep(0.01)
